...
import bestapps.assesmentwipror.interfaces.ItemClickListener;
import bestapps.assesmentwipror.R;
import bestapps.assesmentwipror.adapters.FeedActivityRecyclerAdapter;
import bestapps.assesmentwipror.interfaces.WipServiceInterface;
import bestapps.assesmentwipror.utility.AlertUtil;
import bestapps.assesmentwipror.utility.Utility;
import bestapps.assesmentwipror.remote.ApiUtils;
...
public class MainFeedActivity extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private void getData() {
        {
            mService.getServerData().enqueue(new Callback<Wipro>() {//getFinalData
                @SuppressWarnings("ConstantConditions")
                @Override
                public void onResponse(Call<Wipro> call, Response<Wipro> response) {
                    if (response.isSuccessful()) {
                        //Log.e("response", "response------------------>" + response.body().getRows());
                        mWiproArray = response.body();
                        mWiproStr = mWiproArray.getTitle();
                        mRows = mWiproArray.getRows();
                        try {
                            if (mWiproStr != null) {
                                getSupportActionBar().setTitle(mWiproStr);
                            }
                ...
                @Override
                public void onFailure(Call<Wipro> call, Throwable t) {
